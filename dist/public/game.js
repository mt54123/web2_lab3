...
// tekst
function ispisiTekstCentar(tekst, y, velicina, font, bold = false, italic = false, boja = 'white') {
    let stil = '';
    if (italic)
        stil += 'italic ';
    if (bold)
...
// crtanje palice
function drawPaddle(x, y, width = PADDLE_WIDTH, height = PADDLE_HEIGHT) {
    ctx.fillStyle = "white";
...
// crtanje loptice
function drawBall(x, y, size = BALL_SIZE) {
    ctx.fillStyle = "white";
...
// win screen
function winScreen(playerScore) {
...
function checkHit() {
    for (let brick of bricks) {
        if (brick.destroyed)
            continue;
        const hit = ball.x < brick.x + brick.width &&
